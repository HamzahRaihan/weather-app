...
import useGetWindSpeedAndDirection from '../api/get-windSpeedAndDirection';
...
const WindSpeedAndDirection = () => {
  ...
  const getCurrentHourData = (
    type: 'wind_direction_10m' | 'wind_speed_10m'
  ): number | undefined => {
    const hourlyData = data?.hourly?.[type];
    if (!hourlyData) {
      console.error(`${type.replace(/_/g, ' ')} data is not available.`);
      return undefined;
    }
      ...
      <CardContent>
        <div>
          ...
          <div className="relative flex justify-center align-middle mt-16 items-center h-full w-full">
            ...
            <div className="absolute h-32 w-32 dark:invert">
              <div className="absolute top-1/2 left-0 w-full -translate-y-1/2 transform dark:invert">
                <div className="flex flex-col gap-0 text-xs font-bold text-center">
                  <p>{speed}</p>
                  <p>km/h</p>
                </div>
              </div>
